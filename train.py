...
## 自己加的
def save_embeds(adj, diff, feat, gnn, idx_train, idx_test, sparse):
    model = GCNLayer(input_size, gnn_output_size)  # 1-layer
    model.load_state_dict(gnn.state_dict())
    with torch.no_grad():
        embeds1 = model(feat, adj, sparse)
        embeds2 = model(feat, diff, sparse)
        final_embed=embeds1+embeds2
        final_label=torch.argmax(labels[0, :], dim=1)
    with open("./embeddings.txt", "w") as fe, open("./labels.txt", 'w') as fl:
        for i in range(final_embed.shape[1]):
            fe.write(str(final_embed[0][i][:])+"\n")
            fl.write(str(final_label[i])+"\n")
